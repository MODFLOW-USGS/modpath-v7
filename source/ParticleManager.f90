module ParticleManagerModule
  ...
  use UtilMiscModule,only : rotate_pivot
  implicit none
  ...
  doubleprecision :: xPivot, yPivot, rotAngle
  ...
  subversion = 2
  select type(grid)
	class is (RectangularGridDisvMf6Type)
		xPivot = grid%xPivot
		yPivot = grid%yPivot
		rotAngle = grid%rotAngle
	class default
		xPivot = 0.0d8
	        yPivot = 0.0d8
		rotAngle = 0.0d8
  ...
  do pgIndex = 1, simData%ParticleGroupCount
      do pIndex = 1, simData%ParticleGroups(pgIndex)%TotalParticleCount
			...
			if((p%Status .ne. 0) .and. (p%Status .ne. 8) ) then
				...
				if (rotAngle .ne. 0.0d8) then
					call rotate_pivot(xPivot, yPivot, rotAngle, initialModelX, initialModelY)
				end if
				...
				if (rotAngle .ne. 0.0d8) then
					call rotate_pivot(xPivot, yPivot, rotAngle, modelX, modelY)
				end if
  ...
  subroutine WriteTimeseriesRecord(sequenceNumber, particleID, groupIndex,      &
    timeStep, timePointIndex, pCoord, geoRef, outUnit, grid)
  use ModflowRectangularGridModule,only : ModflowRectangularGridType
  use RectangularGridDisvMf6Module,only : RectangularGridDisvMf6Type
  implicit none
  ...
  ! un-rotate only if necessary
  if (rotAngle .ne. 0.0d0) then
	call rotate_pivot(xPivot, yPivot, rotAngle, modelX, modelY)
  end if
  !!
  ...
  subroutine WritePathlineRecord(tpResult, outUnit, stressPeriod, timeStep, geoRef, grid)
  ...
  subroutine WriteBinaryPathlineRecord(tpResult, outUnit, stressPeriod, timeStep, geoRef, grid)
  use TrackPathResultModule,only : TrackPathResultType
  use ParticleCoordinateModule,only : ParticleCoordinateType
  use ModflowRectangularGridModule,only : ModflowRectangularGridType
  use RectangularGridDisvMf6Module,only : RectangularGridDisvMf6Type
